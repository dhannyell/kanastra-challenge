...
def test_put_upload_file_success(save_debits_mock, upload_file, app):
...
@mock.patch.object(DebitUseCase, "save_debits")
def test_put_upload_file_success_with_no_insert(save_debits_mock, upload_file, app):
...
@mock.patch.object(DebitUseCase, "save_debits")
def test_put_upload_file_error_invalid_file(save_debits_mock, upload_file, app):
    save_debits_mock.return_value = None
...
def test_put_upload_file_error_invalid_header(upload_file, app):
    response = app.put(
        "/api/upload-file",
        data={"file": upload_file},
    ...
    assert response.json == {
        "message": "Invalid Content-Type. Include multipart/form-data in Header"
    }


@mock.patch.object(DebitUseCase, "save_debits")
def test_put_upload_file_error_generic_exception(save_debits_mock, upload_file, app):
